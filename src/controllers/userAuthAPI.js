...
export const signUp = async (req, res) => {
    try {
        const { userName, email, password, layout, gender, image, aboutYourself } = req.body;
        if (!userName) {
...
export const getUser = async (req, res) => {
    try {
        ...
        res.status(400).json({
            message: err instanceof Error ? err.message : "Failed to get user details.",
        });
    }
};
...
export const updateUser = async (req, res) => {
    try {
        ...
        if (layout !== undefined)
            loggedInUser.layout = layout;
        if (aboutYourself !== undefined)
            loggedInUser.aboutYourself = aboutYourself;
        // CHECK FOR FILE
...
//# sourceMappingURL=userAuthAPI.js.map